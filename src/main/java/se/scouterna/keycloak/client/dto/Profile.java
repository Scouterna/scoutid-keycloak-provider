...
public class Profile {
    ...
    private static String formatNameForEmail(String name) {
        ...
        formatted = formatted.replaceAll("\\s+", "."); // Spaces → dots
        formatted = formatted.replace(".-", "-").replace("-.", "-"); // Replace -. and -. with -
        formatted = Normalizer.normalize(formatted, Normalizer.Form.NFKD)
            .replaceAll("\\p{M}", "")           // Remove diacritics
            .replace("æ", "ae").replace("Æ", "AE")
            .replace("ß", "ss")
            .replace("ø", "o").replace("Ø", "O")
            .replace("ł", "l").replace("Ł", "L"); // Polish L with stroke
        formatted = formatted.replaceAll("[^a-zA-Z0-9.\\-]", ""); // Keep only alphanumeric, dot, hyphen
